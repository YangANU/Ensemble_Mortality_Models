...
# Visualising MSE and MAE for Japan and US 
...
combined_names = colnames(get(comparison_mse_female[country_index])$combined[,3:8])

data_input = data.frame(h = 1:10, cbind(get(comparison_mse_female[country_index])$base[1:10,], 
                                        get(comparison_mse_female[country_index])$combined[,3:8])) |> pivot_longer(-h)

ggplot(data_input, aes(x = h, y = value, colour = name,  linetype = name, linewidth = name)) +
  geom_line() + 
  theme(legend.position = "bottom") +
  ggtitle("Japanse Female") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Forecast Horizon (h)") + ylab("MSE x 100") + 
  scale_x_continuous("Forecast Horizon (h)", limits = c(1,10), breaks = 1:10) +
  scale_linetype_manual(values = c(rep(2:4, each = 4), 6, 6, 6,rep(1,6)), breaks = c(all_method, combined_names)) +
  scale_shape_manual(values = c(rep(1,15), rep(2,6)), breaks = c(all_method, combined_names)) +
  scale_linewidth_manual(values = c(rep(1,15), rep(1.5,6)), breaks = c(all_method, combined_names)) +
  scale_color_manual(values = c(hcl.colors(25, "YlOrRd")[1:15],  hcl.colors(10, "Blues")[1:6]), breaks = c(all_method, combined_names)) +
  labs(color = "Method", linetype = "Method", linewidth = "Method") +
  guides(linetype = guide_legend(title = "Method", nrow = 4, byrow = FALSE))

ggplot(data_input, aes(x = h, y = value, colour = name,  linetype = name, linewidth = name)) +
  geom_line() + 
  theme(legend.position = "none") +
  ggtitle("Japanse Female (zoomed in)") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Forecast Horizon (h)") + ylab("MSE x 100") + 
  scale_x_continuous("Forecast Horizon (h)", limits = c(1,10), breaks = 1:10) +
  scale_linetype_manual(values = c(rep(2:4, each = 4), 6, 6, 6,rep(1,6)), breaks = c(all_method, combined_names)) +
...
combined_names = colnames(get(comparison_mse_male[country_index])$combined[,3:8])

data_input = data.frame(h = 1:10, cbind(get(comparison_mse_male[country_index])$base[1:10,], 
                                        get(comparison_mse_male[country_index])$combined[,3:8])) |> pivot_longer(-h)

ggplot(data_input, aes(x = h, y = value, colour = name,  linetype = name, linewidth = name)) +
  geom_line() + 
  theme(legend.position = "bottom") +
  ggtitle("Japanse Male") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Forecast Horizon (h)") + ylab("MSE x 100") + 
  scale_x_continuous("Forecast Horizon (h)", limits = c(1,10), breaks = 1:10) +
  scale_linetype_manual(values = c(rep(2:4, each = 4), 6, 6, 6,rep(1,6)), breaks = c(all_method, combined_names)) +
...
ggplot(data_input, aes(x = h, y = value, colour = name,  linetype = name, linewidth = name)) +
  geom_line() + 
  theme(legend.position = "none") +
  ggtitle("Japanse Male (zoomed in)") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Forecast Horizon (h)") + ylab("MSE x 100") + 
  scale_x_continuous("Forecast Horizon (h)", limits = c(1,10), breaks = 1:10) +
  scale_linetype_manual(values = c(rep(2:4, each = 4), 6, 6, 6,rep(1,6)), breaks = c(all_method, combined_names)) +
